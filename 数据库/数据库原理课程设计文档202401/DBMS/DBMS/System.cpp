...
void myDBMS::openDataBase(string dataBaseName) {
	...
	string pathName = "C:\\Users\\24208\\Downloads\\数据库原理课程设计文档202401\\数据库原理课程设计文档202401\\" + dataBaseName;
	if (0 != _access(pathName.c_str(), 0))printf("该数据库不存在");
	else {
		prePath = pathName + "\\";
		cout << "打开数据库成功" << endl;
		open = true;
...
void myDBMS::myCreateDataBase(string dataBaseName) {//建立数据库
	...
	if (0 != _access(pathName.c_str(), 0)) {
		if (0 == _mkdir(pathName.c_str()))//返回0表示创建成功,-1表示失败
			cout << "创建成功" << endl;
		else
			cout << "创建失败" << endl;
		return;
