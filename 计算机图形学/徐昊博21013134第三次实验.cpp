...
void triangle (GLsizei mode)
{
    ...
    if(mode==1)
    {
        ...
        glLoadIdentity();
        glOrtho(-5.0,5.0,-5.0,5.0,-5,5);//定义左视区的三维正交观察可视体，创建一个正射投影矩阵
    ...
    {
        setLookAt(iMode,angle,s_eye,s_at);//根据菜单iMode选择的三视图，根据右手原则，
        //围绕某坐标轴旋转angle，建立视角视觉效果
        glMatrixMode(GL_PROJECTION);
        glLoadIdentity();
        gluPerspective(45,aspect,-5,5);//定义右视区的三维透视观察可视体
        objectID=getTetrahedron(A, B, C, D);
        axisID=getAxis(3.0);
        glCallList(objectID);
        glCallList(axisID);
...
int main(int argc, char* argv[])
{
    ...
    glutCreateWindow("徐昊博21013134第三次实验");
